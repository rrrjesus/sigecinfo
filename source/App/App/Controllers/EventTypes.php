...
class EventTypes extends Admin
{
    ...
    public function list(): void
    {
        ...
        $head = $this->seo->render("Tipos de Evento - " . CONF_SITE_NAME, CONF_SITE_DESC, url("/app/tipos-de-eventos"), null, false);

        $breadcrumb = [
            ["title" => "Eventos", "link" => url("/app/tipos-de-eventos")],
            ["title" => "Listar"]
        ];
    ...
    public function disabledList(): void
    {
        ...
        $head = $this->seo->render("Tipos de Evento Desativados - " . CONF_SITE_NAME, CONF_SITE_DESC, url("/app/tipos-de-eventos"), null, false);
    ...
    public function create(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "create") {
            ...
            header('Content-Type: application/json');
            $json["redirect"] = url("/app/tipos-de-eventos");
            echo json_encode($json);
            return;
        }

        $breadcrumb = [
            ["title" => "Tipo de Evento", "link" => url("/app/tipos-de-eventos")],
            ["title" => "Criar"]
        ];

        $head = $this->seo->render("Registar Tipo de Evento - " . CONF_SITE_NAME, CONF_SITE_DESC, url("/app/tipos-de-eventos"), null, false);
    ...
    public function edit(array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "update") {
            ...
            header('Content-Type: application/json');
            $json["redirect"] = url("/app/tipos-de-eventos/editar/{$eventType->id}");
            echo json_encode($json);
            return;
        }
        ...
        $head = $this->seo->render("Editar Tipo de Evento: {$eventType->name}", CONF_SITE_DESC, url("/app/tipos-de-eventos"), null, false);
