...
class Events extends Admin
{
    ...
    public function processCheckInFromPage(array $data): void
    {
        ...
        try {
            ...
            redirect(url("/painel/eventos/portaria/{$participant->event_id}"));
    ...
    public function qrCodeCheckIn(array $data): void
    {
        ...
        $breadcrumb = [
            ["title" => "Meus Eventos", "link" => url("/painel/eventos/meus-eventos-agendados")],
            ["title" => $event->title],
        ...
        $qrCodeUrl = url("/painel/eventos/checkin-qr-scan/{$participant->id}/{$participant->event_id}/{$secureToken}");

        $qrCode = new \Source\Support\QrCode();
        $qrCodeSvg = $qrCode->svg($qrCodeUrl);

        $head = $this->seo->render("QR Code Check-in: " . $participant->user()->user_name, CONF_SITE_DESC, url("/painel"), null, false);
    ...
    public function listEventsDisableds(): void
    {
        ...
        $head = $this->seo->render("Eventos Finalizados - " . CONF_SITE_NAME, CONF_SITE_DESC, url("/painel/eventos/finalizados"), null, false);
        
        $breadcrumb = [
            ["title" => "Eventos Finalizados", "link" => url("/painel/eventos/finalizados")],
            ["title" => "Listar"]
        ];
    ...
    {
        $head = $this->seo->render("Eventos Agendados - " . CONF_SITE_NAME, CONF_SITE_DESC, url("/painel/meus-eventos"), null, false);
        
        $eventRepo = new EventRepository();
        $myEvents = $eventRepo->getEventsForUser($this->user->id) ?? [];

        $breadcrumb = [
            ["title" => "Eventos", "link" => url("/painel/eventos/eventos")],
    ...
    public function justify(array $data): void
    {
        ...
        if (!$participantId) {
            $json["message"] = message("error", "Participante inválido.");
        ...
        if (!$participant || $participant->user_id != $this->user->id) {
            $json["message"] = message("error", "Você não tem permissão para justificar esta participação.");
        ...
        if (empty($justification)) {
            $json["message"] = message("warning", "Por favor, escreva o motivo da sua ausência.");
        ...
        if ($participant->save()) {
            ...
            $json["message"] = message("error", "Ocorreu um erro ao salvar. Tente novamente.");
    ...
    public function completedEvents(): void
    {
        ...
        $head = $this->seo->render("Meu Histórico de Eventos - " . CONF_SITE_NAME, CONF_SITE_DESC, url("/painel/eventos/meus-eventos-finalizados"), null, false);
